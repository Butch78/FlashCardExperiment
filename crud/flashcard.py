...
def get_flashcard(flashcard_id: str) -> Flashcard:
    ...
    with psycopg2.connect(url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, front, back, section_id, user_id, created_at FROM pilot_flashcards WHERE id = %s",
                (flashcard_id,),
            )
...
def get_flashcards(review_section_id: str) -> List[Flashcard]:
    ...
    with psycopg2.connect(url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, front, back, file_name, section_heading, section_id, user_id, created_at from pilot_flashcards WHERE section_id = %s",
                (review_section_id,),
            )
...
def create_review(review: Review):
    ...
    with psycopg2.connect(url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO pilot_reviews (id, flashcard_id, file_name, accuracy, comphrension, relevance) VALUES (%s, %s, %s, %s, %s, %s)",
                (
                    review.id,
                    review.flashcard_id,
                    review.file_name,
